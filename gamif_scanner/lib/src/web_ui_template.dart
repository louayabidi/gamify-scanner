...
.foolish-in {
  animation: foolishIn 1s both;
    ...
    .badge-skip {
      background: rgba(124,122,153,0.08);
      color: var(--muted);
      border: 1px solid rgba(124,122,153,0.15);
    }
...
async function scan() {
  ...
  const btnContent = document.getElementById('scanBtnContent');
  ...
  try {
    ...
    setTimeout(() => area.classList.remove('error'), 2000);
  }
}
...
function renderList() {
  ...
  for (const [file, list] of Object.entries(byFile)) {
    ...
    const rel = idx >= 0 ? parts.slice(idx).join('/') : file;
...
function getSelected() {
  return [...document.querySelectorAll('[data-m]:checked')]
    .map(c => JSON.parse(c.getAttribute('data-m')));
}
...
function all()   { document.querySelectorAll('[data-m]').forEach(c => c.checked = true);  updateSel(); }
function none()  { document.querySelectorAll('[data-m]').forEach(c => c.checked = false); updateSel(); }
function smart() {
  document.querySelectorAll('[data-m]').forEach(c => {
    const m = JSON.parse(c.getAttribute('data-m'));
...
async function inject() {
  ...
  const btnContent = document.getElementById('injectBtnContent');
  ...
  try {
    ...
    const initLine = '<div class="log-line"><span class="log-prefix">✓</span><span>GamifSDK.init() → main.dart</span></div>';
    const logLines = selected.map(m =>
      '<div class="log-line"><span class="log-prefix">✓</span><span>GamifTracker.track(\\'' + m.name + '\\')</span></div>'
    ).join('');
    const errLines = r.errors.map(e =>
      '<div class="log-line log-err"><span class="log-prefix">✕</span><span>' + e + '</span></div>'
    ).join('');
